pro getcompdata2,mdir=mdir,datacomp,time,tottime
...
datacomp=create_struct('dete',intarr(2),'deter',intarr(2),'en',fltarr(2),'pos',fltarr(2,2),'ang',0.,'time',0.d,'flag','')  
...
;create temporary structures
    datact1=create_struct('dete',intarr(2),'deter',intarr(2),'en',fltarr(2),'pos',fltarr(2,2),'ang',0.,'time',0.d,'flag','')  
...
nel=n_elements(isgrpha)
   for i=0L,nel-1L do begin
      ...
      if (isgrpha[i] lt picspha[i]) then begin
         ...
         datact[i].deter=[isgry[i]+(isgrz[i]*128L),picsity[i]+(picsitz[i]*64L)]
         ...
         datact[i].deter=[picsity[i]+(picsitz[i]*64L),isgry[i]+(isgrz[i]*128L)]
