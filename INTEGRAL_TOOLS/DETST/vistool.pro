pro vistool,chi2,ps=ps,file=file
...
if (NOT keyword_set(file)) THEN file='vistool.ps'
...
oplot,[rad*cos(!PI/6.),rad*cos(!PI/6.)]+coord[0,i],[-rad*cos(!PI/3.),rad*cos(!PI/3.)]+coord[1,i]
   for j=0,5 do begin
     ang=(!PI/6.)+(j*(!PI/3.))
     if chi2(i,j) ne 0. then begin
        num=num+1
        totchi2=totchi2+chi2(i,j)
        col=256-(floor(chi2(i,j))*2)-55
