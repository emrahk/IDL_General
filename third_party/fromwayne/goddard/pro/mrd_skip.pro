...
;       of bytes.    GZIP files opened with /COMPRESS are also read as a series
;       of bytes, since this is faster than using  POINT_LUN when not at the 
;       beginning of a file. 
; CALLING SEQUENCE:
;       MRD_SKIP, Unit, Nskip
;
...
;  We test for the presence of gzip compressed files using the .compress field 
;  of fstat(unit).   For Unix compress  we test whether we can read the current
;  position.    If we can then it is not a pipe and we can  
