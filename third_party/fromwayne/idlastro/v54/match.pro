...
;       MATCH
; PURPOSE:
;       Routine to match values in two vectors.   **V5.4 or later**
;
; CALLING SEQUENCE:
...
;       V5.4 version uses COMPLEMENT to WHERE  W. Landsman   April 2002
;-
;-------------------------------------------------------------------------
 On_error,2
 ...
 sa = size(a,/struc) & sb = size(b,/struc)
 da = sa.type & db = sb.type                         ;IDL Data type numbers
 ...
 na = sa.N_Elements              ;number of elements in a
 nb = sb.N_Elements              ;number of elements in b
