...
; 5/9/95 Kills old widget efficiently and without remorse
; 11/10/95 Accumulates livetime individually
; Do some preliminary stuff first
;*********************************************************************
...
if (xregistered("wam")ne 0 or xregistered('wfit') ne 0)$
...
wam = {	base:0L}
wam.base = widget_base(title = ' ARCHIVE MULTISCALAR DATA',$
           /frame,/column)
;**********************************************************************
...
if (det lt 5)then begin
   c(*,*) = cnts(opt-3,det - 1,*,*) & tme = time(opt-3,*)
endif else begin
...
;*********************************************************************
wtable,wam.base,d,idfs_string,idfe_string,cp,string(tme)
;*********************************************************************
; Create the plotting area in the lower left
;*********************************************************************
wplot,wam.base,400,400,draw,row3
;*********************************************************************
...
;**********************************************************************
if (ft)then begin
   xpdmenu,['"RETURN"{','"TO ARCMSCLR"','}'],rcol1
   xtle = 'FREQUENCY (S!E-1!N)'
   ytle = 'POWER RATIO (P!Is!N/P!In!N)'
   tle = strcompress('FFT, ' + tle)
   ntbins = n_elements(cc)
   x = findgen(ntbins/2)/(ntbins*ltime)
   cc_ = cc
...
;**********************************************************************
; Do button for fft of data
;**********************************************************************
   if (fold gt 0)then begin
      xpdmenu,$
      ['"RETURN"{','"TO FFT"','"TO FOLD"','"TO ARCMSCLR"','}'],rcol1
      per = periods(fold-1)
      ...
      w1 = widget_button(rcol1,value = 'FFT')
      x = ltime*findgen(ntbins) & yy = cc
      xtle = 'TIME (SEC)'
      ytle = 'COUNTS'
      yymin = 0
...
; Realize the widgets
;**********************************************************************
widget_control,wam.base,/realize
...
hstplot,low,high,yy,xtle,ytle,tle,stle,xrnge,yrnge
nz = where(yy ne 0)
