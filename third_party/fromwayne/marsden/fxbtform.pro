	PRO FXBTFORM,HEADER,TBCOL,IDLTYPE,FORMAT,NUMVAL,MAXVAL
;+
; Project     : SOHO - CDS
;
; Name        : 
...
;
	IF N_PARAMS() LT 1 THEN MESSAGE,	$
		'Syntax:  FXBTFORM,HEADER,TBCOL,IDLTYPE,FORMAT,' + $
		'NUMVAL,MAXVAL'
;
;  Get the number of fields.
;
	TFIELDS = FXPAR(HEADER,'TFIELDS')
	IF !ERR LT 0 THEN MESSAGE,	$
		'Invalid FITS header -- keyword TFIELDS is missing'
	IF TFIELDS EQ 0 THEN MESSAGE,	$
		'FIT binary table has no columns'
;
...
;
	...
	IF !ERR LT 0 THEN MESSAGE,	$
		'Invalid FITS table header -- keyword TFORM not present'
...
NEXT_CHAR:
		IF ICHAR GE LEN(I) THEN MESSAGE,	$
			'Invalid format specification for keyword TFORM ' + $
			STRTRIM(I+1)
...
;
			'M':  BEGIN
				IDLTYPE(I) = 5
				IF MAXVAL(I) GT 0 THEN MAXVAL(I)=MAXVAL(I)*2 $
						  ELSE NUMVAL(I)=NUMVAL(I)*2
				WIDTH(I) = 8
				END
;
			ELSE:  MESSAGE,'Invalid format specification ' + $
