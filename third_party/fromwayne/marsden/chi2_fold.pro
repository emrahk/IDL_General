...
;       fdot.........1st frequency derivative
;      ffdot.........2nd frequency derivative
...
if (n_elements(counts) eq 0)then begin
   print,'USAGE: chi2_fold,counts,time,prange,chi2,' + $
         ...
         '[dof=# degrees of freedom],[silent=(boolean)],[fdot=(0)],' + $
         '[ffdot=(0)]'
   return
endif
;********************************************************************
...
if (n_elements(periods) ne 0)then begin
   nperiods = double(n_elements(periods))
      
endif else periods = min(prange) + $
                    dp*dindgen(nperiods)/(nperiods-1d) + $
                    dp/(2d * nperiods)
...
for i = 0,nperiods - 1 do begin
   ...
   fold_time_arr,time,counts,periods(i),flc,np=nbins,pd=pdot,$
                 pp=ppdot,fd=fdot,ff=ffdot
;********************************************************************
