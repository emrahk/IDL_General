...
;
	IF N_PARAMS() LT 4 THEN MESSAGE, $
		'Syntax:  FXBWRITE, UNIT, DATA, COL, ROW'
;
...
;
	IF (COL LT 1) OR (COL GT TFIELDS(ILUN)) THEN MESSAGE,	$
		'COL must be between 1 and ' + STRTRIM(TFIELDS(ILUN),2)
	IF (ROW LT 1) OR (ROW GT NAXIS2(ILUN)) THEN MESSAGE,	$
		'ROW must be between 1 and ' + STRTRIM(NAXIS2(ILUN),2)
;
...
;
	IF MAXVAL(ICOL,ILUN) GT 0 THEN BEGIN
		IF N_ELEMENTS(DATA) GT MAXVAL(ICOL,ILUN) THEN MESSAGE,	$
			'Data array should have no more than ' +	$
			STRTRIM(N_ELEM(ICOL,ILUN),2) + ' elements'
	END ELSE BEGIN
		IF N_ELEMENTS(DATA) NE N_ELEM(ICOL,ILUN) THEN MESSAGE,	$
			'Data array should have ' +	$
			STRTRIM(N_ELEM(ICOL,ILUN),2) + ' elements'
	ENDELSE
;
...
;
	IF MAXVAL(ICOL,ILUN) GT 0 THEN BEGIN
		...
		IF FORMAT(ICOL,ILUN) EQ "M" THEN N_ELEM0 = N_ELEM0 / 2
		IF FORMAT(ICOL,ILUN) EQ "X" THEN BEGIN
			IF N_ELEMENTS(BIT) EQ 0 THEN MESSAGE,	$
				'Number of bits not defined'
			IF N_ELEMENTS(BIT) NE 1 THEN MESSAGE,	$
				'Number of bits must be a scalar'
			IF LONG((BIT+7)/8) NE N_ELEM0 THEN MESSAGE,	$
				'Number of bits does not match array size'
			N_ELEM0 = BIT
		ENDIF
;
