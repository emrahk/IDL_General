...
;
	IF N_PARAMS() LT 2 THEN MESSAGE, $
		'Syntax:  FXWRITE, FILENAME, HEADER  [, DATA ]'
;
...
;
	IF N_PARAMS() EQ 2 THEN BEGIN
		IF (FXPAR(HEADER,'NAXIS') NE 0) THEN MESSAGE,	$
			'NAXIS should be zero for no primary data array'
		IF (NOT FXPAR(HEADER,'EXTEND')) THEN MESSAGE,	$
			'EXTEND should be true for no primary data array'
	END ELSE BEGIN
		CHECK_FITS, DATA, HEADER, /FITS
		IF !ERR NE 0 THEN MESSAGE,'Error occurred checking FITS header'
	ENDELSE
;
