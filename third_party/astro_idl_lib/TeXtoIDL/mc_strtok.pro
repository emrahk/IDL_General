...
;       Revision 1.4 2001/01/11 Joern Wilms
;          wilms@astro.uni-tuebingen.de
;       changed name from strtok to mc_strtok to avoid name clash with
;       internal (?) IDL function strtok
;
...
;-
FUNCTION mc_Strtok, string, token, $
                 TRIM=trim, HELP=Help
...
; Back to the caller if error occurs.
    ...
    IF (n_params() NE 2) OR keyword_set(Help) THEN BEGIN 
        ...
        print, offset+'new = strtok( old, token )'
        print, offset+'Inputs:'
        ...
        print, offset+offset+"If old is 'foo44 bar', then strtok( old, '44' ) would return'"
...
;'
        print, offset+offset+"If old='xyz', then new=strtok(old,'a') would return with"
