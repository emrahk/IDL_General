...
;
; CALLING SEQUENCE:
;       vso_print_struct, structure, Tags_to_print [ , title, string_matrix 
...
;       Forked as 'vso_print_struct', J.Hourcle Nov 2011:
;         added /LEFT to get left aligned strings
;-

pro vso_print_struct, structure, Tags_to_print, title, string_matrix, TNUMS=tagi, $
 ...
 ;variables	 
	 ...
	 Ntag = N_tags(structure)
        ...
        for i=0,Npr-1 do begin
                ...
                vtypes[i] = st[st[0]+1]
                CASE vtypes[i] OF
                1:      formats[i] = "I" + justify + strtrim( ncht[i]>5, 2 ) + ")"
                2:      formats[i] = "I" + justify + strtrim( ncht[i]>8, 2 ) + ")"
                3:      formats[i] = "I" + justify + strtrim( ncht[i]>12, 2 ) + ")"
                7: BEGIN
                        sLens[i] = $
                         ( max( strlen( structure.(tagi[i]) ) ) + 2 ) > ncht[i]
                        formats[i] = "A" + justify + strtrim( sLens[i], 2 ) + ")"
                     END
                else: BEGIN
