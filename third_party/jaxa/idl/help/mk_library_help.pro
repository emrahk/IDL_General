; $Id: mk_library_help.pro,v 1.2 1994/01/13 21:39:26 ali Exp $
...
;
  ...
  if (found) then begin
    ...
    txt_pos = txt_pos + 3
    found = 0
    while (not found) and (not eof(in_file)) do begin		;MDM added "not eof(in_file)"
      readf, in_file, tmp
...
pro MK_LIBRARY_HELP, source, outfile, verbose=verbose, title=title
  ...
  if (vms_tlb) then begin
    ...
    if (!version.os eq 'vms') then begin
      ...
      tmp = '/'
    endelse
    ;;files = findfile(source + tmp + '*.pro', count=count)		;MDM removed 30-Jun-95
    files = file_list(source, '*.pro', file=rname)
    ss = sort(rname)	& rname = rname(ss)	& files = files(ss)	;MDM 30-Jun-95 alphabetize
    ss = uniq(rname)	& rname = rname(ss)	& files = files(ss)	;MDM 30-Jun-95 remove duplicates
    if (files(0) eq '') then count = 0 else count = n_elements(files)
    if (!version.os eq 'vms') then begin
...
;;	  strlen(name) - prefix_length-4))
          subject = strupcase(strmid(rname(i), 0, strlen(rname(i)) -4))
      endelse
