...
;-
;
;  The following dummy routines allow TVZOOM2 to work with IDL VMS executables
;  that do not support widgets.  They are only there to allow TVZOOM2 to compile
...
;
;******************************************************************************
;			 TVZOOM2 widget event handler.
;******************************************************************************
;
PRO TVZOOM2_EVENT, EVENT
;
...
;
;******************************************************************************
;			    Main TVZOOM2 procedure.
;******************************************************************************
;
PRO TVZOOM2, IMAGE, X0, X1, Y0, Y1, XSIZE=XS, YSIZE=YS, FACT=FACT, $
            INTERP=INTERP, CONTINUOUS=CONT, NOSCALE=NOSCALE,	$
            TITLE=TITLE, MISSING=MISSING, MAX=MAX, MIN=MIN, TOP=TOP, $
            BOTTOM=BOTTOM, VELOCITY=VELOCITY, COMBINED=COMBINED, LOWER=LOWER, $
            KEEP=KEEP, ZOOM_WINDOW=ZOOM_WIN,NEW_WINDOW=NEW_WIN,DISABLE=DISABLE,$
            RECURSIVE=RECURSIVE, group=group
;
   ON_ERROR,2                   ;Return to caller if an error occurs

;-- added by DMZ to handle case where external widget programs (such as
;   IMAGE_TOOL) kill TVZOOM widget and cause it to crash
...
;
   ...
   IF N_ELEMENTS(TITLE) EQ 0 THEN TITLE = 'TVZOOM2'
;
...
;  the widget version of TVZOOM2 on that image.
;
   IF N_PARAMS() EQ 0 THEN BEGIN
      IF !D.NAME EQ 'WIN' THEN BEGIN
         A = TVREAD(DISABLE=DISABLE)
         TVZOOM2, A, XSIZE=XS, YSIZE=YS, FACT=FACT,	$
...
;
      ...
      BSCALE,IM,NOSCALE=NOSCALE,MISSING=MISSING,MAX=MAX,MIN=MIN, $
         TOP=TOP,BOTTOM=BOTTOM,VELOCITY=VELOCITY,COMBINED=COMBINED,LOWER=LOWER
      ...
      IF N_ELEMENTS(group) NE 0 THEN $
         XMANAGER,'TVZOOM2',BASE, group_leader = group, $
         modal = KEYWORD_SET(group) $
      ELSE $
         XMANAGER,'TVZOOM2',BASE
