...
; EXPLANATION:
;	NOTE: ssw_legend was copied from $SSW/gen/idl_libs/astron/plot/legend.pro
;	and renamed to ssw_legend in order to preserve the version of legend in use
;	prior to the conversion to AL_LEGEND, which uses the Coyote Graphics Routines
;	(CG had a few problems with old code related to the Z buffer and decomposed
;       color, which D.F. fixed, but there might be other issues), since the legend 
;	routine in astron will be deleted (see next note).
...
; CALLING SEQUENCE:
;       SSW_LEGEND [,items][,keyword options]
; EXAMPLES:
;       The call:
;               ssw_legend,['Plus sign','Asterisk','Period'],psym=[1,2,3]
;         produces:
;               -----------------
;               |               |
;               |  + Plus sign  |
...
;       ssw_legend,items,linestyle=lines            ; vertical legend---upper left
;       items = ['Plus sign','Asterisk','Period']
;       sym = [1,2,3]
;       ssw_legend,items,psym=sym                   ; ditto except using symbols
;       ssw_legend,items,psym=sym,/horizontal       ; horizontal format
;       ssw_legend,items,psym=sym,box=0             ; sans border
;       ssw_legend,items,psym=sym,delimiter='='     ; embed '=' betw psym & text
;       ssw_legend,items,psym=sym,margin=2          ; 2-character margin
;       ssw_legend,items,psym=sym,position=[x,y]    ; upper left in data coords
;       ssw_legend,items,psym=sym,pos=[x,y],/norm   ; upper left in normal coords
;       ssw_legend,items,psym=sym,pos=[x,y],/device ; upper left in device coords
;       ssw_legend,items,psym=sym,/position         ; interactive position
;       ssw_legend,items,psym=sym,/right            ; at upper right
;       ssw_legend,items,psym=sym,/bottom           ; at lower left
;       ssw_legend,items,psym=sym,/center           ; approximately near center
;       ssw_legend,items,psym=sym,number=2          ; plot two symbols, not one
;       ssw_legend,items,/fill,psym=[8,8,8],colors=[10,20,30]; 3 filled squares
; INPUTS:
...
;           c2_line = [0,2,1]
;           ssw_legend,c1_items,psym=c1_psym,corners=c1,box=0
;           ssw_legend,c2_items,line=c2_line,corners=c2,box=0,pos=[c1[2],c1[3]]
...
;           ssw_legend,['1','22','333','4444'],linestyle=indgen(4),corners=corners
...
;           ssw_legend,['1','22','333','4444'],linestyle=indgen(4),pos=pos
...
;	Renamed ssw_legend from legend and moved from astron to ssw, K Tolbert 6-Jan-2012 
;-
pro ssw_legend, items, BOTTOM_LEGEND=bottom, BOX = box, CENTER_LEGEND=center, $
...
compile_opt idl2
on_error,2
if keyword_set(help) then begin & doc_library,'ssw_legend' & return & endif
;
...
if n eq 0 then message,'No inputs!  For help, type ssw_legend,/help.'
if ni eq 0 then begin
  items = replicate('',n)                               ; DEFAULT BLANK ARRAY
endif else begin
  if size(items,/TNAME) NE 'STRING' then message, $
      'First parameter must be a string array.  For help, type ssw_legend,/help.'
...
case n_elements(position) of
 ...
 else: message,'Position keyword can have 0, 1, or 2 elements only. Try ssw_legend,/help.'
endcase
...
;; extra by djseed .. psym=88 means use the already defined usersymbol
 ...
 if psym[i] EQ 10 then $
         message,'PSYM=10 (histogram mode) not allowed to ssw_legend.pro' $
