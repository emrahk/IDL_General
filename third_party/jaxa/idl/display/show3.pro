; $Id: show3.pro,v 1.9 1999/01/16 01:25:49 scottm Exp $
;
...
PRO show3, image, x, y, INTERP = interp, SSCALE=sscale, E_CONTOUR=ec, $
	E_SURFACE=es, bottom=bottom, top=top
...
if not exist(top) then top = !d.table_size-1 else top = top
top = top < (!d.table_size - 1)
...
if (!d.flags and 1) eq 1 then begin	;Scalable pixels (PostScript)?
	...
	miss = top		;Missing values are white
	c_color=[bottom,bottom]		;Contour in only one color, black
 endif else begin
	fact = 1 		;one pixel/output coordinate
	miss = 0		;missing is black
	c_color=[150,200,top-5]
 endelse

if (!d.flags and 512) ne 0 then $  ;White background?
	miss = top else miss = bottom
;
