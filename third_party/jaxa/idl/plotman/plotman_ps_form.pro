...
; changed if statement to check for the orientation state of the widget, mimster@stars.gsfc.nasa.gov
IF info.landscape EQ 1 THEN BEGIN

   landscape = info.landscape  ; set the default to the widget state, mimster@stars.gsfc.nasa.gov
   ...
   coords = PS_Form_PlotBox_Coords(ysize, xsize, yoff, xoff, inches)  ; set for defaults for landscape orientation, mimster@stars.gsfc.nasa.gov
   ...
   xsizeStr = String(ysize, Format='(F5.2)')
   ysizeStr = String(xsize, Format='(F5.2)')
   xoffStr =  String(yoff, Format='(F5.2)')
   yoffStr =  String(xoff, Format='(F5.2)')
...
Function Plotman_PS_Form, xoffset, yoffset, Cancel=cancelButton, Help=help, $
   ...
   Create=createButton, Parent=parent, no_filename=no_filename
...
IF landscape THEN Begin
 IF inches THEN pagesize = 11.0 ELSE pagesize = 11.0 * 2.54
...
buttonbase = Widget_Base(tlb, row=1,/frame)
accept = Widget_Button(buttonbase, Value='Accept', UValue='ACCEPT')

cancel = Widget_Button(buttonbase, Value='Cancel', UValue='CANCEL')
;   create = Widget_Button(buttonbase, Value='Create File', UValue='CREATE')

;defbuttonbase = Widget_Base(buttonbase, Row=1)
   sysdefaults = Widget_Button(buttonbase, Value='System Defaults', $
      Event_Pro='PS_Form_Set_System_Defaults', UValue='SYSTEM')
   localdefs = Widget_Button(buttonbase, Value='Local Defaults', $
...
visible=(1-keyword_set(no_filename))
filenamebase = Widget_Base(tlb, Column=1,map=visible)
...
filenamew = Widget_Text(fbase, /Editable,xsize=30,  $
...
;IF thisRelease EQ '5' THEN XManager, 'ps_form', tlb ELSE $
      XManager, 'ps_form', tlb, /Modal,group=parent
