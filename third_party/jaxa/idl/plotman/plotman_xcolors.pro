...
;        01-Aug-2002 by Kim. Renamed to hsi_xcolors.  Modified to put HESSI color tables first in list.
;          Since colors are not shown in order of index number any more, had to save colorNamesIndex in
;          info structure.
;          Took out of SSW gen tree and put in HESSI tree.  Also this file had its own version of
;          CONGRID included that was out of date.  Removed it so it will use the standard CONGRID.
;        07-Jul-2008, Kim. Renamed to plotman_xcolors (for move of plotman to ssw/gen)
;-
;
;###########################################################################
;
...
PRO PLOTMAN_XCOLORS_COLORTABLE, event	;kim 1-aug-02

   ; Get the info structure from storage location.

Widget_Control, event.top, Get_UValue=info, /No_Copy

selected = info.colorNamesIndex[event.index]	; kim 1-aug-02
index = fix( (ssw_strsplit(selected, ' -', /head))[0])	; kim 1-aug-02

LoadCT, index, File=info.file, /Silent, $	; kim 1-aug-02
...
info.gamma = 1.0
info.index = index	; kim 1-aug-02
info.ctname = info.colornames[index]	; kim 1-aug-02

   ; Create a pseudo structure.

pseudo = {currenttop:info.currenttop, currentbottom:info.currentbottom, $
   reverse:info.reverse, windowindex:info.windowindex, index:index, $	; kim 1-aug-02
...
PRO PLOTMAN_XCOLORS, NColors=ncolors, Bottom=bottom, Title=title, File=file, $
...
; find colors with HESSI in the name.  Put them first, then put the rest of the list after that.
q = where (strpos(colorNames, 'HESSI') ne -1, count)	; added this block. kim 1-aug-02
if count gt 0 then begin
	temp = colorNamesIndex
	remove, q, temp
	colorNamesIndex = [colorNamesIndex[q], temp]
endif
filebase = Widget_Base(tlb, Column=1, /Frame)
listlabel = Widget_Label(filebase, Value='Select Color Table...')
list = Widget_List(filebase, Value=colorNamesIndex, YSize=12, Scr_XSize=256, $
   Event_Pro='PLOTMAN_XColors_ColorTable')	;kim 1-aug-02
...
info = {  windowIndex:windowIndex, $         ; The WID of the draw widget.
          ...
          colorNamesIndex:colorNamesIndex, $ ; array of strings- color index followed by name	; kim 1-aug-02
