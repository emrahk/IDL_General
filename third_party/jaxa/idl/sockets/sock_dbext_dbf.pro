...
if sflag then begin
 ...
 url_dbf=getenv('ZDBASE')+'/'+name+'.dbf'
 sass->set,file=url_dbf,data=bytarr(len)
endif
        ...
        if dbno eq 0 then begin
                for i=0L,nlist-1 do begin
                    if list[i] ge 0 then begin
                     if sflag then entry=sass->read(list[i]) else entry=qdbrec[list[i]]
                     big[0,i] = entry[index]
                    endif
                endfor
        endif else begin      ;mapped I/O
                unit=db_info('unit_dbf',dbno)
                rec_size=db_info('length',dbno)
                for i=0L,nlist-1 do begin
                    if list[i] ge 0 then begin
                     data=bytarr(rec_size,/nozero) & offset=rec_size*list[i]
                     if sflag then begin
                      sass->set,data=data,offset=offset
                      entry=sass->read(0)
                     endif else begin
                      p=assoc(unit,data,offset)
                      entry=p[0]
                     endelse
                     big[0,i] = entry[index]
                    endif
