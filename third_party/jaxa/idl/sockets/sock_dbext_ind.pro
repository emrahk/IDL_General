...
;
 ...
 if sflag then begin
  sass->set,data=lonarr(7,h[0]),off=8
  header=sass->read(0)
 endif else begin
 ...
 if Nindex LT 1 then begin
  message,'Item not indexed, DBNO may be wrong',/info
  goto,bail
 endif
...
if sflag then begin
 if dtype NE 7 then begin
  if numvals GT 1 then $ 
   sass->set,data=make_array(size=[2,numvals,nv,dtype,0],/NOZERO), off=sbyte else $
    sass->set,data=make_array(size=[1,nv,dtype,0],/NOZERO),off=sbyte 
 endif else sass->set,data=make_array(size=[2,nbytes,nv,1,0],/NOZERO), off=sbyte
endif else begin
 if dtype NE 7 then begin
  if numvals GT 1 then $ 
...
;
if sflag then values=sass->read(0) else values = p[0]
...
bail: if sflag then obj_destroy,sass
