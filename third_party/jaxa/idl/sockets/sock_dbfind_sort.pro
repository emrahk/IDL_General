...
name=strlowcase(db_info('NAME',0))

sflag=is_url(getenv('ZDBASE'))
if sflag then begin
 url_dbx=getenv('ZDBASE')+'/'+name+'.dbx'
 sass=obj_new('sock_assoc')
 sass->set,file=url_dbx,data=lonarr(2)
 h=sass->read(0)
endif else begin
...
if sflag then begin
 sass->set,data=lonarr(7,h[0]),offset=8
 header=sass->read(0)
endif else begin
...
if dtype ne 7 then data=make_array( size=[1,nv,dtype[0],0],/NOZERO) else $
 data=make_array( size=[2,nbytes,nv,1,0],/NOZERO)
...
if sflag then begin
 sass->set,data=data,offset=sbyte
 values=sass->read(0)
endif else begin
 p=assoc(unit,data,sbyte)
 values=p[0]
endelse
...
number=last-first+1
if dtype NE 7 then data=make_array(size=[1,number,dtype,0],/nozero) else $
 data=make_array( size=[2,nbytes,number,1,0],/NOZERO)

if sflag then begin
 sass->set,data=data,offset=sbyte+(first-1)*num_bytes
 values=sass->read(0)
endif else begin
 p = assoc(unit,data,sbyte+(first-1)*num_bytes) 
 values=p[0]
endelse
...
;
        data=make_array(size=[1,number,3,0],/nozero)
        if sflag then sass->set,data=data,offset=sbyte+(first-1)*4 else $
         p = assoc(unit,make_array(size=[1,number,3,0],/nozero),sbyte+(first-1)*4)

        if list[0] EQ -1 then begin
                if sflag then list=sass->read(0) else list=p[0]
                if external then byteorder,list, /NTOHL
        endif else begin
                if sflag then list2=sass->read(0) else list2=p[0]
