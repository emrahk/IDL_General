function gt_sum_l, item, header=header, string=string, short=short, spaces=spaces, title=title
;
;+
;NAME:
;	gt_sum_l
...
;-
;
title = 'HXT Low Channels (14-23 keV)'
header_array = "Sum Low"
...
if (typ eq 8) then begin
    ...
    tags = tag_names(item)
    case tags(0) of
	'GEN':		out = item.hxt.sum_l
	'ENTRY_TYPE':	out = fix(item.hxt_sum_l)^2		;data is stored compressed
	else:		out = item.sum_l
    endcase
end else begin
    out = item
