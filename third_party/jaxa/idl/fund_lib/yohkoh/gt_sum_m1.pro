function gt_sum_m1, item, header=header, string=string, short=short, spaces=spaces, title=title
;
;+
;NAME:
;	gt_sum_m1
...
;-
;
title = 'HXT Medium 1 Channels (23-33 keV)'
header_array = "Sum M1 "
...
if (typ eq 8) then begin
    ...
    tags = tag_names(item)
    case tags(0) of
	'GEN':		out = item.hxt.sum_m1
	'ENTRY_TYPE':	out = fix(item.hxt_sum_m1)^2		;data is stored compressed
	else:		out = item.sum_m1
    endcase
end else begin
    out = item
end
