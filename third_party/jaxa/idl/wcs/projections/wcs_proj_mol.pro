...
;-
;
pro wcs_proj_mol, wcs, coord, missing=k_missing
on_error, 2
halfpi = !dpi / 2.d0
;
...
test0 = 2 - y^2
factor = sqrt(test0>0)
w = where(factor eq 0, count)
if count gt 0 then factor[w] = 1
phi = !dpi * x / (2 * factor)
test1 = y/sqrt(2)
test2 = asin(-1>test1<1)/halfpi + (y/!dpi)*factor
theta = asin(-1>test2<1)
w_missing = where((abs(test1) gt 1) or (abs(test2) gt 1) or $
                  (abs(phi) gt !dpi) or (test0 le 0), n_missing)
;
