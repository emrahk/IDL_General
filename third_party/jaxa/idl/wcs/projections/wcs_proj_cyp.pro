...
;
;               NOWRAP     = If set, then don't wrap the longitude values to
;                            be between +/- 180 degrees.
;
...
;               Version 2, 27-Mar-2013, WTT, Added keyword NOWRAP
;
; Contact     :	WTHOMPSON
;-
;
pro wcs_proj_cyp, wcs, coord, missing=k_missing, nowrap=nowrap
on_error, 2
halfpi = !dpi / 2.d0
;
...
;
;  Get the projection source distance mu, and the cylindrical radius lambda.
;  If not found, use the default values of 1.
;
mu = 1.d0
if tag_exist(wcs, 'proj_names', /top_level) then begin
...
if tag_exist(wcs, 'proj_names', /top_level) then begin
    ...
    w = where(wcs.proj_names eq name, count)
    if count gt 0 then lambda = wcs.proj_values[w[0]]
endif
;
;  Make sure that the mu and lambda values are valid.
;
if lambda le 0 then begin
    message, /continue, 'Lambda must be > 0 -- resetting to 1'
    lambda = 1
endif
if mu eq -lambda then begin
    message, /continue, 'Mu must not be -lambda -- resetting to 1'
    mu = 1
endif
;
;  Calculate the native spherical coordinates.
;
phi = cx*coord[wcs.ix,*] / lambda
eta = cy*coord[wcs.iy,*] / (mu + lambda)
test = eta*mu/sqrt(eta^2+1)
theta = atan(eta,1) + asin(-1>test<1)
w_missing = where((abs(test) gt 1) or (abs(theta) gt halfpi), n_missing)
;
...
if ~keyword_set(nowrap) then begin
    ...
    if count gt 0 then alpha[w] = alpha[w] + twopi
