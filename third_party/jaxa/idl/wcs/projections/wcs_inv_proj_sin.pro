...
;-
;
pro wcs_inv_proj_sin, wcs, coord
on_error, 2
halfpi = !dpi / 2.d0
;
...
;
cos_theta = cos(theta)
if (zeta ne 0) or (eta ne 0) then sin_theta = sin(theta)
if zeta eq 0 then x = cos_theta * sin(phi) else $
  x =  cos_theta*sin(phi) + zeta*(1 - sin_theta)
if eta eq 0 then y = -cos_theta * cos(phi) else $
  y = -cos_theta*cos(phi) +  eta*(1 - sin_theta)
;
