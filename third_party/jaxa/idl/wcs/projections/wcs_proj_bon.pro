...
;-
;
pro wcs_proj_bon, wcs, coord, missing=k_missing
on_error, 2
halfpi = !dpi / 2.d0
;
...
if tag_exist(wcs, 'proj_names', /top_level) then begin
    ...
    if count gt 0 then theta1 = wcs.proj_values[w[0]]
endif
if n_elements(theta1) eq 0 then begin
    message, /continue, 'Theta1 parameter not passed -- defaulting to 45 deg'
    theta1 = 45
end else if theta1 eq 0 then begin
    message, /continue, 'Theta1=0 not allowed -- defaulting to 45 deg'
    theta1 = 45
endif
theta1 = theta1 * (!dpi / 180.d0)
;
...
x = cx*coord[wcs.ix,*]
y = cy*coord[wcs.iy,*]
y0 = theta1 + 1.d0/tan(theta1)
r_theta = sqrt(x^2 + (y0-y)^2)
i_missing = r_theta eq 0
w_missing = where(i_missing, n_missing)
if n_missing gt 0 then r_theta[w_missing] = 1
if theta1 lt 0 then r_theta = -r_theta
aphi = atan(x/r_theta, (y0-y)/r_theta)
theta = y0 - r_theta
cos_theta = cos(theta)
w = where(cos_theta eq 0, count)
if count gt 0 then cos_theta[w] = 1
phi = aphi * r_theta / cos_theta
i_missing = i_missing or (abs(phi) gt !dpi) or (abs(theta) gt halfpi)
w_missing = where(i_missing, n_missing)
;
