...
;-
;
pro wcs_inv_proj_ait, wcs, coord
on_error, 2
halfpi = !dpi / 2.d0
;
...
;
gamma = 1.d0 + cos(theta)*cos(phi/2.d0)
w_missing = where(gamma le 0, n_missing, complement=w1, ncomplement=n1)
if n1 gt 0 then gamma[w1] = sqrt(2.d0 / gamma)
x = 2.d0 * gamma * cos(theta) * sin(phi / 2.d0)
y = gamma * sin(theta)
;
