...
;-
;
pro wcs_proj_coo, wcs, coord, missing=k_missing
on_error, 2
halfpi = !dpi / 2.d0
;
...
x = cx*coord[wcs.ix,*]
y = cy*coord[wcs.iy,*]
theta1 = thetaA - abs(eta)
theta2 = thetaA + abs(eta)
tan1 = tan((halfpi - theta1) / 2.d0)
tan2 = tan((halfpi - theta2) / 2.d0)
if theta1 eq theta2 then c = sin(theta1) else $
  c = alog(cos(theta2)/cos(theta1)) / alog(tan2/tan1)
if tan1 eq 0 then psi = cos(theta2) / (c*tan2^c) else $
  psi = cos(theta1) / (c*tan1^c)
y0 = psi * tan((halfpi - thetaA) / 2.d0)^c
...
theta = halfpi - 2*atan((r_theta/psi)^(1.d0/c))
;
