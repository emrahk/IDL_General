...
;       This routine has been rewritten to use the built-in routine FILE_INFO
;       for better cross-platform compatibility, with the output restructured
;       for backwards compatibility.  For versions of IDL earlier than 5.5, the
;       routine works as described below.
;
...
;       Version 3, 5-Nov-2008, William Thompson, rewrote to use FILE_INFO.
;       Version 3.1, 10-Nov-2008, WTT, fixed bug when filename is blank
;
; Version: Version 3, 5-Nov-2008
;
;-----------------------------------------------------------------------------
...
if !version.release ge '5.5' then begin
    filename = file[0]
    if filename eq '' then filename = '.'       ;blank is current directory
    info = file_info(filename)
    exist   = fix(info.exists)
    read    = fix(info.read)
    write   = fix(info.write)
    execute = fix(info.execute)
    if info.directory then filetype = 'd' else filetype = '-'
    return
endif
