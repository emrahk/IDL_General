pro icsf, wav, int, wavf, intf, int_cor,                            $
		  ...
		  gauss=gauss, coeff_final=coeff_final
;+
...
; ICSF.PRO corrects the intensity using an iterative procedure and cubic spline, as described in 
; Klimchuk, Patsourakos, & Tripathi (2015, ApJ, submitted). The corrected intensity equals the "true" 
; intensity at bin center, rather than the average over the bin. A Gaussian fit can be used instead of 
; a cubic spline by setting the keyword gauss (/gauss in the calling sequence). Other fitting functions 
; can be used by editing the lines of code marked with %%%%%.
...
;    gauss = set to iterate over a Gaussian fit instead of a cubic spline fit
...
;    coeff_final = Gaussian parameters of final iteration
...
;    2015-sep-03, added the /gauss keyword, JAK
;-
...
; F (initial spline fit, fine resolution)
...
;    %%%%% The following can be edited to use a different fitting function   
      yfit = gaussfit(wav, int, coeff, nterms=3)           ; initial Gaussian fit
      i0 = coeff(0)
      x0 = coeff(1)
      w = sqrt(2.)*coeff(2)
      intf_all(0,*) = i0*exp(-(wavf - x0)^2/w^2)
;    %%%%%
   endelse
...
;       %%%%% The following can be edited to use a different fitting function   
	     yfit = gaussfit(wav, int_new_all(i,*), coeff, nterms=3)    
         i0 = coeff(0)
         x0 = coeff(1)
         w = sqrt(2.)*coeff(2)
         intf_all(i,*) = i0*exp(-(wavf - x0)^2/w^2)					 
;       %%%%%
      endelse	
...
; Final Gaussian parameters
   if keyword_set(gauss) then coeff_final = coeff
