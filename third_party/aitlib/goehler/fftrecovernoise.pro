function fftrecovernoise, time, rate, fourier_zero_index=fourier_zero_index
;+
; NAME:
;             fftrecovernoise
;
...
;            zero fourier components, here for noisy data 
;
; CATEGORY: 
;             timing tools
;
...
IF n_elements(time) EQ 0 THEN BEGIN 
  simdat,time,rate, noise=0.0, num=1000,gap_level=1.5

  ;; create gaps:
  gap_index = indgen(20)*10+200
  ...
  fourier_zero_index    = indgen(n_elements(gap_index))*2 + 40
  fourier_data_index    = indgen(n_elements(gap_index))*2+41


;  d = (fft(rate))[fourier_zero_index]
...
IF n_elements(fourier_zero_index) EQ 0 THEN $
  fourier_zero_index    = indgen(m)*2+full_data_n/2 - m +1


;; default index of nonzero parts of fourier components close to zero
;; fourier components:
IF n_elements(fourier_data_index) EQ 0 THEN $
  fourier_data_index    = indgen(m)*2+full_data_n/2 - m +2
...
;; check: fourier zero components must equal number of gaps:
IF n_elements(fourier_data_index) NE m THEN $
  message, "Error: Number of reference frequencies does not match to gap number"
...
;; a_21 - n X n
exponent =   (data_index # fourier_data_index   / full_data_n) MOD 1.D0
a21 = exp(dcomplex(0.D0,-2.D0*!DPI * exponent))/(full_data_n)

;; a_22 - n X m
exponent =   (gap_index # fourier_data_index   / full_data_n) MOD 1.D0
...
z =  reform((a21-a11)##rate)
;z= reform(a11##rate)



;; solve A11*rate + A12*y = 0:
full_data[gap_index] =  invert((a12-a22),stat,/double)##z;lu_complex((a12-a22),z ,/double)
;full_data[gap_index] =  lu_complex(a12,-z ,/double)

;d1 = (fft(full_data))[fourier_zero_index]
return, full_data
