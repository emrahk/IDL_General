...
;
; DESCRIPTION:
;           Shift left, till data point in current group found. If no
;           data left present do nothing. If shift distance is
;           less than 1/4 of screen we move 1/4 of screen left.
...
;
; HISTORY:
;           $Id: cafe_wplot_seekleft.pro,v 1.10 2003/04/28 07:45:10 goehler Exp $
;             
;-
;
; $Log: cafe_wplot_seekleft.pro,v $
; Revision 1.10  2003/04/28 07:45:10  goehler
; new parameter setting scheme: parameters are set as in usual functions
;
; Revision 1.9  2003/04/24 09:55:10  goehler
; Report actions done into log file. This allows reprocessing log files as batch files.
;
; Revision 1.8  2003/04/07 12:50:39  goehler
...
;
; Revision 1.4  2002/09/20 07:48:52  goehler
; typos
;
...
PRO cafe_wplot_seekleftevent, ev
    ...
    ;; for all subgroups look at closest left data point:
    FOR i = 0, n_elements((*env).groups[group].data)-1 DO BEGIN 
        ...
        index = where(*(*env).groups[group].data[i].x LT (*env).plot.xpos)

        ;; none found
        IF index[0] EQ -1 THEN CONTINUE
        
        ;; find minimum
        new_xpos = new_xpos > max((*(*env).groups[group].data[i].x)[index])
    ENDFOR 
    ...
    ENDIF ELSE BEGIN 
        ...
        IF ((*env).plot.xpos - new_xpos) LT 0.25*(*env).plot.xwidth THEN $
...
;; ------------------------------------------------------------
;; SEEKLEFT - THE MAIN FUNCTION WHICH ALLOCATES A NEW BUTTON
;; ------------------------------------------------------------

PRO cafe_wplot_seekleft,env,  baseID,  help=help, shorthelp=shorthelp
    ...
    IF keyword_set(shorthelp) THEN BEGIN  
        print, "seekleft   - seek left button"
        return
    ENDIF


    buttonID= widget_button(baseID, value="  <-  ", $
                            event_pro="cafe_wplot_seekleftevent")
