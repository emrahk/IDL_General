...
;
; DESCRIPTION:
;           This buttons action will enlarge the displayed
;           x-range. It will be assured that the range is either
;           1,2,5 times a power of 10.
;           Example: The xrange is 5*10^3 -> zoom out will set the
;           x range at 1*10^4.
;           This allows an almost equal scaled plot for different
;           data sets.
;           
; PARAMETER:
;           /t3d - zoom the y axis as well as the x axis.
;               
; SIDE EFFECTS:
;           Changes xrange of displayed plot (affects yrange also when
;           t3d selected).
;
;
; HISTORY:
;           $Id: cafe_wplot_zoomout.pro,v 1.13 2003/04/28 07:45:12 goehler Exp $
;             
;-
;
; $Log: cafe_wplot_zoomout.pro,v $
...
PRO cafe_wplot_zoomoutevent, ev
    ...
    CASE round(10.D^(alog10((*env).plot.xwidth)$
        ...
        2 : (*env).plot.xwidth = (*env).plot.xwidth * 5.D0 / 2.D0
        5 : (*env).plot.xwidth = (*env).plot.xwidth * 2.D0
...
;; ------------------------------------------------------------
;; CAFE_WPLOT_ZOOMOUT3D --- EVENT PROCEDURE FOR 3-DIM zooming
;; ------------------------------------------------------------

PRO cafe_wplot_zoomoutevent3d, ev
    ...
    CASE round(10.D^(alog10((*env).plot.ywidth)$
        ...
        2 : (*env).plot.ywidth = (*env).plot.ywidth * 5.D0 / 2.D0
        5 : (*env).plot.ywidth = (*env).plot.ywidth * 2.D0
...
;; ------------------------------------------------------------
;; ZOOMOUT - THE MAIN FUNCTION WHICH ALLOCATES A NEW BUTTON
;; ------------------------------------------------------------

PRO cafe_wplot_zoomout,env,  baseID,t3d=t3d,  help=help, shorthelp=shorthelp
    ...
    IF keyword_set(shorthelp) THEN BEGIN  
        print, "zoomout   - zoom out button"
        return
    ENDIF


    IF (keyword_set(t3d)) THEN BEGIN 
        buttonID= widget_button(baseID, value="zoom out", $
                                event_pro="cafe_wplot_zoomoutevent3d")
    ENDIF ELSE BEGIN 
        buttonID= widget_button(baseID, value="zoom out", $
                                event_pro="cafe_wplot_zoomoutevent")
    ENDELSE 
