...
;
; PURPOSE:
;           Button to change viewport to left
;
; CATEGORY:
;           cafe
;
...
;
; DESCRIPTION:
;           Step viewed portion left by 10% of displayed plot range.
;               
; SIDE EFFECTS:
...
;
; HISTORY:
;           $Id: cafe_wplot_stepleft.pro,v 1.9 2003/04/28 07:45:11 goehler Exp $
;             
;-
;
; $Log: cafe_wplot_stepleft.pro,v $
; Revision 1.9  2003/04/28 07:45:11  goehler
; new parameter setting scheme: parameters are set as in usual functions
;
; Revision 1.8  2003/04/24 09:55:11  goehler
...
;
; Revision 1.6  2003/03/17 14:11:42  goehler
...
PRO cafe_wplot_stepleftevent, ev
    ...
    ;; set new x position:
    (*env).plot.xpos =            $
      (*env).plot.xpos - 0.1*(*env).plot.xwidth
...
;; ------------------------------------------------------------
;; STEPLEFT - THE MAIN FUNCTION WHICH ALLOCATES A NEW BUTTON
;; ------------------------------------------------------------

PRO cafe_wplot_stepleft,env,  baseID,  help=help, shorthelp=shorthelp
    ...
    IF keyword_set(shorthelp) THEN BEGIN  
        print, "stepleft   - step left button"
        return
    ENDIF


    buttonID= widget_button(baseID, value="  <|  ", $
                            event_pro="cafe_wplot_stepleftevent")
