...
;
; SYNTAX:
;           select, range [,subgroup][,group]
;
; INPUTS:
;           range    - Defines range of data points to select. This can
;                      be either:
...
;                         data point will be selected. 
;                         If <val2> is less than <val1> all but the
;                         interval <val2>..<val1> is selected.  
;                         Examples:
;                               > select, 2
;                                -> select data point 2 (the 3-rd)
;                               > select, 5-22
;                                -> select data points 5..22 (starting from 0)
;                               > select 46-*
;                                -> select all data points upwards
;                                   from point 47.  
;                       - X value range, denoted by float point numbers:
;                         <val1>-<val2>,
;                         while <val1>, <val2> represents X values
;                         defining the interval to select. Open
...
;                         Examples:
;                               > select, 2.2-5.7
;                                 -> select all data points from x=2.2 to 5.7
;                               > select, *-0.2
;                                 -> select all data points up to x=0.2
;                               > select, 0.5-0.2
;                                 -> select all data points except
;                                    those in 0.2..0.5. 
;                         
;                       - Boolean expressions with X/Y values. All
;                         values for which this expression is true are
;                         selected. The expression may contain
...
; EXAMPLE:
;
;               > select, 0.2-7.5,  "test.dat", 3
;                 -> select all x values between 0.2 and 7.5 from
;                 group 3, subgroup represented by file "test.dat"
;
; HISTORY:
;           $Id: cafe_select.pro,v 1.6 2003/03/17 14:11:35 goehler Exp $
;-
;
; $Log: cafe_select.pro,v $
; Revision 1.6  2003/03/17 14:11:35  goehler
; review/documentation updated.
;
; Revision 1.5  2003/03/03 11:18:25  goehler
; major change: environment struct has become a pointer -> support of wplot/command line
; in common.
; Branch to be able to maintain the former line also.
;
; Revision 1.4  2002/09/19 14:02:38  goehler
; documentized
;
...
;
    ...
    IF keyword_set(shorthelp) THEN BEGIN  
        cafereport,env, "select   - select datapoints for further processing"
        return
    ENDIF
    ...
    ;; ------------------------------------------------------------
    ;; SELECT DATAPOINTS FOR EACH SUBGROUP
    ;; ------------------------------------------------------------
    ...
    FOR i =0,n_elements(subgroup)-1 DO BEGIN 
        ...
        IF index[0] NE -1 THEN BEGIN 
            ...
            (*(*env).groups[group].data[sg].selected)[index] = 1
