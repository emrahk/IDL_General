	PRO JWFXBREAD, UNIT, DATA, COL, ROW, NOSCALE=NOSCALE, VIRTUAL=VIR, $
...
;
;jw	IF MAXVAL(ICOL,ILUN) GT 0 THEN BEGIN
...
;
		IF FORMAT(ICOL,ILUN) EQ 'M' THEN DIMS = [2,DIMS]
;
...
;
	IF (N_ELEMENTS(DIMENSIONS) GT 0) AND (FORMAT(ICOL,ILUN) NE 'X')	$
			THEN BEGIN
		IF FORMAT(ICOL,ILUN) EQ 'M' THEN	$
			DIMS0 = [2,DIMENSIONS] ELSE DIMS0 = DIMENSIONS
