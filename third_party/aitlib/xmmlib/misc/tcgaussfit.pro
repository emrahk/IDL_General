...
Function tcgaussfit, x, y, a, NTERMS=nt, ESTIMATES = est, $
                     PWEIGHTS=PWEIGHTS, WEIGHTS=WEIGHTS, SIGMAA=SIGMAA, CHISQ=CHISQ
;+
; NAME:
;	TCGAUSSFIT
;
...
;       PWEIGHTS: use weights for poisson distribution, i.e. w(x)=1/y(x)   
;
; OUTPUTS:
...
;       SIGMAA: Vector of standard deviations of the elements of A
;       CHISQ:  CHISQ of fit   
;
; COMMON BLOCKS:
;	None.
...
;       T.Clauss, Apr, 2000, Added PWEIGHTS, WEIGHTS, SIGMA and CHISQ keywords 
;-
...
IF NOT(keyword_set(WEIGHTS)) THEN BEGIN
    IF keyword_set(PWEIGHTS) THEN BEGIN
        weights=1/y             ; ??
;       weights=y
    ENDIF ELSE BEGIN
...
return,curvefit(x,y,weights,a,sigmaa, $
		function_name = "GAUSS_FUNCT",chisq=chisq) ;call curvefit
end
