PRO CCD_FLUX, ima, rad, xcen, ycen, flux, area, MAXINT=maxint, $
...
;
; CALLING SEQUENCE:
;	CCD_FLUX, ima, rad ,xcen ,ycen , MAXINT=maxint, $
...
if not EXIST(quad) then begin
   ...
   then begin
      ...
      if ind(0) ne -1 then begin
         ...
         flux=total(image(ind))/double(sub)^2
         maxint=max(image(ind))
      endif else begin
         area=0.0d0
         flux=0.0d0
         maxint=0.0d0
   ...
   then begin
      ...
      flux=total(ima(xmi:xma,ymi:yma))
