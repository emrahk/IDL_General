; $Id: ccd_curve.pro,v 1.1.1.1 1999/10/14 13:17:23 cvs Exp $
FUNCTION CCD_CURVE, x, y, w, a, sigmaa, Function_Name = Function_Name, $
...
;
; CALLING SEQUENCE:
;       Result = CCD_CURVE(X, Y, W, A, SIGMAA, FUNCTION_NAME = name, $
...
;    
;       R.D.GECKELER : IAA, Dec. 18 1996 : Fixed bug. If all_done,
;                      goto statement resulted in correct yfit values,
;                      but wrong returned parameters b.
;	    
;-
...
;      Define the partial derivative array
       if double then pder = fltarr(n_elements(x), nterms) $
	else pder = dblarr(n_elements(x), nterms)
;
