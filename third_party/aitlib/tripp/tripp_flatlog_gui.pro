FUNCTION TRIPP_FLATLOG_GUI, log
;+
; NAME:
;           TRIPP_FLATLOG_GUI
;
;
; PURPOSE:
;           Definition of a graphical user interface for the flat
;           version of the log files needed by the TRIPP routines
;
;
; INPUTS:
;           log: structure (type: tripp_flatlog_type) containing either default 
;           values or values from a previous session. 
;
;
; RESTRICTIONS:
;           structure: TAG NAMES in log must correspond to those
;                      defined in tripp_flatlog_type.
;
;   
; OUTPUT:
;           result: Structure (type: tripp_flatlog_type) containing the flat
;                   version of the tripp_flatlog_type structure.
;
;
...
;-
  ...
  gui_sheet = [ '1, BASE,,ROW',        $
                ...
                '0,TEXT   ,'+STRING(log.zero)+',    LABEL_LEFT=Name of zero to subtract OR the keyword "overscan" : ,' $
                + 'WIDTH=55, TAG=zero', $
