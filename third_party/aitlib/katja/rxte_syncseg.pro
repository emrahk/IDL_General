PRO rxte_syncseg,path,channels, $
		 hexte=hexte,bkg=bkg,back_p=back_p,back_m=back_m, $
                 ...
                 chatty=chatty,novle=novle,gz=gz
;+
; NAME:
;          rxte_syncseg  
;
...
FOR i=0,nch-1 DO BEGIN
    ...
    IF keyword_set(hexte) THEN BEGIN
        ...
        IF keyword_set(bkg) THEN BEGIN 
            ...
            IF gz then spawn,['/usr/bin/find',lcroot,'-name', $
                   '*'+'bkg_'+bin+'_'+channels(i)+'.lc.gz'], $
              lclist,/noshell else spawn,['/usr/bin/find',lcroot,'-name', $
