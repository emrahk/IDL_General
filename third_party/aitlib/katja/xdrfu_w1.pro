...
;          frequency array, the multidimensional quantity
;          array (with each set calculated from one lightcurve), and
;          the history keyword to a file in xdr format 
;
;
; FEATURES: 
;          write a multidimensional Fourier quantity, i.e., the
...
;          defined by one parameter (e.g., the energy band of the
;          corresponding lightcurve); the order and IDL formats of the
;          output components are documented in the ASCII file
...
;
; CALLING SEQUENCE:
;          xdrfu_w1,funame,inpfreq,inpquan, $
;            history=history,high=inphigh, $
;            chatty=chatty,gzip=gzip  
;
;
...
;                     that is to be written in xdr format
...
;                     defined by one parameter; quan=float(inpquan) is
;                     written to funame   
;
;   
; OPTIONAL INPUTS:
...
;          inpfreq=fr
;          inpquan=fltarr(n_elements(fr),2)
;          inpquan[*,0]=psd0
;          inpquan[*,1]=psd1  
;          xdrfu_w1,'test.psd',inpfreq,inpquan, $
;                  history='Test',/chatty   
;
;
...
;                                  IDL header: minor changes   
;          Version 1.5, 2000/10/30 Katja Pottschmidt,   
;                                  IDL header: minor changes
;          Version 1.6, 2001/02/08 Katja Pottschmidt,
;                                  input keyword ``high'' added     
;                                  and new version number ('xdrfu1 1.1')
;                                  introduced   
;          Version 1.7, 2001/02/20 Katja Pottschmidt,
;                                  a gzipped file can now also be written: 
;                                  the gzip keyword has to be set
;          Version 1.8, 2001/07/10 Katja Pottschmidt,
...
;; write version
version='xdrfu1 1.0'
IF (n_elements(high) NE 0) THEN version='xdrfu1 1.1'
writeu,unit,version
...
nch=n_elements(quan(0,*))     & writeu,unit,nch
