...
IF (keyword_set(evt)) THEN BEGIN
    FOR i=0,numfiles do begin
        ...
        ;; Make sure no bad times came through
        ;;
        w=where(rawtime GT 0, wcq)
        IF (wcq ne 0) then begin
            rawtime=rawtime(w)
        ENDIF
        w=' '
        ...
        IF (keyword_set(norb) eq 0) THEN BEGIN
           ;; Correct for the system's barycenter
           print,'Removing the binary orbit'
           rawtime=(temporary(rawtime)+timezero)/86400.00d + mjdref  ; convert to days
           rawtime=removeorb(rawtime,asini,porb,t90,ecc,omega)       ; remove the orbit
        ENDIF

        IF (i EQ 0) THEN BEGIN
            goodtime=[rawtime]
        ENDIF ELSE BEGIN
            goodtime=[goodtime,rawtime]
        ENDELSE
...
ENDIF ELSE BEGIN
    FOR i=0,numfiles DO BEGIN
        ...
        IF (keyword_set(rate)) THEN BEGIN
            cnt=fits_get(hd,tab,'RATE')
        ENDIF ELSE BEGIN
            cnt=fits_get(hd,tab,'COUNTS')
        ENDELSE

        ;; Sometimes invalid (negative) times come through. Remove them
        ;; Get rid of all empty bins
        ;;
        w=where(rawtime GT 0 AND cnt GT 0, wcq)
        IF (wcq ne 0) then begin
            rawtime=rawtime(w)
            cnt=cnt(w)
        ENDIF
...
;;
;; Make the time array symmetric around 0 to remove phase as an
;; interesting parameter
;;
goodtime=temporary(goodtime)*86400.00d      ; convert to secs
center=(max(goodtime)+min(goodtime))/2.0d   ; calculate midpoint of array
goodtime=temporary(goodtime)-center         ; recenter
