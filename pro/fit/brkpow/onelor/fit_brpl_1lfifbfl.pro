pro fit_brpl_1lfifbfl,p,p_err,f_b,a1,fi,fb,fl,fit,guess
...
modelname = 'brk_pow_1lfifbfl' ; the model name should also go here
...
c1_2 = fl*(fb^(-fi))*(x(g2)^fi)
oplot, x(g2), c1_2, linestyle = 2, thick = 1

c2_1 = (a(0)*a(1))/(2.0*!pi*den(g1))
oplot, x(g1), c2_1, linestyle = 2, thick = 1
c2_2 = (a(0)*a(1))/(2.0*!pi*den(g2))
oplot, x(g2), c2_2, linestyle = 2, thick = 1

 print,'which parameter you want to change, 4 to change index, or 6 to exit'
 read,ind
 case ind of
  0: begin
     print,a(0)
  ...
  3: begin
     print,fl,'  remember this is going to be fixed in curvefit'
     read,var
     fl=var
  end
...
print, 'frac_rms = ', sqrt(fl*fb+(a(0)*fb^(-fi)*(100^(fi+1)-fb^(fi+1))/(fi+1))), '  ???'
print, 'break f = ', fb, '  fixed'
print, 'level = ', fl
print, 'index = ', fi, '  fixed'
print, 'f_buency = ', a(2), sigmaa(2)
print, 'frac_rms = ', sqrt(a(0)), sigmaa(0)/(2.0*sqrt(a(0)))
