pro fit_brpl_1lfifffb,p,p_err,f_b,a1,a2,fi,ff,fw,fb,fit
...
modelname = 'brk_pow_1lfifffb' ; the model name should also go here
...
print, 'frac_rms = ', sqrt(a(0)*fb+(a(0)*fb^(-fi)*(100^(fi+1)-fb^(fi+1))/(fi+1))), sigmaa(0)/(2.0*sqrt(a(0)))
