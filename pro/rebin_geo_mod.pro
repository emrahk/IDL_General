...
pro rebin_geo_mod, step, freq, p, p_err,p_err_em
...
while (ihigh lt nf) do begin
 ...
 if (ihigh lt nf) then begin
   freq2(i) = total(freq(ilow:ihigh))/float(rfactor) ; the average frequency
   p2(i) = total(p(ilow:ihigh))/float(rfactor) ; the average power
          ...
          p2_err_em(i) = (total(p_err(ilow:ihigh))/float(rfactor))/sqrt(float(rfactor))
 endif
  endif
  i = i + 1
endwhile
